...
public class DDBReader {
	...
	private static final Logger LOGGER = Logger.getLogger(SQSAccess.class
			.getName());
	...
	public String reqCellStatNet(String reqId, String begin, String end,
			String cellid) {
		...
		try {
			...
			return RequestID.createError("XMLError", begin,
					"Impossible to parse this date" + e.getMessage())
					.toString();
		}
		...
		try {
			...
			return RequestID.createError("XMLError", end,
					"Impossible to parse this date" + e.getMessage())
					.toString();
		}
		...
		Condition condition = new Condition().withComparisonOperator(
				...
				new AttributeValue().withN(tbegin.toString() + "000").withN(
						tend.toString() + "000"));
		...
		QueryRequest queryRequest = new QueryRequest()
				...
				.withAttributesToGet(
						Arrays.asList("timestamp", "carType", "data"));
		...
		firstTime = firstTime.substring(0, firstTime.length() - 4);// remove the
																	// 3 last
																	// digit
																	// (nodeid)
		firstTime = dateFormatter.format(new Date(Long.parseLong(firstTime)));
		String lastType = result.getItems().get(result.getCount() - 1)
				.get("carType").getN();
		String lastTime = result.getItems().get(result.getCount() - 1)
				.get("timestamp").getN();
		lastTime = firstTime.substring(0, lastTime.length() - 4);// remove the 3
																	// last
																	// digit
																	// (nodeid)
		...
		return RequestID.createReplyCellStatNet(reqId, cellid, begin, end,
				firstType, firstTime, lastType, lastTime, numberOfData,
	...
	public String reqCellStatSpeed(String reqId, String begin, String end,
			String cellid) {
		...
		for (Map<String, AttributeValue> item : result.getItems()) {
			if (item.get("flags").getB().get() == 0) {
				CellDirection cd = listSpeedside1.get(item.get("carType")
						.getN());
				if (cd == null) {
				...
				CellDirection cd = listSpeedside2.get(item.get("carType")
						.getN());
				if (cd == null) {
					cd = new CellDirection(item.get("carType").getN());
		...
		ArrayList<CellDirection> toCompute1 = new ArrayList<CellDirection>(
				listSpeedside1.values());
		ArrayList<CellDirection> toCompute2 = new ArrayList<CellDirection>(
				listSpeedside2.values());

		return RequestID.createReplyCellStatSpeed(reqId, cellid, begin, end,
				toCompute1, toCompute2).toString();
	}
